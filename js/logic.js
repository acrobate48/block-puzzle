...
function clearLines(grid){
  const rf=[],cf=[];
  for(let r=0;r<ROWS;r++)if(grid[r].every(v=>v))rf.push(r);
  for(let c=0;c<COLS;c++){let f=true;for(let r=0;r<ROWS;r++)if(!grid[r][c]){f=false;break;}if(f)cf.push(c);}
  const cl2=new Set(),clC={};
  // Constraint block 2-hit mechanic:
  //   __BLOCKED__ in a full line → crack it (becomes __CRACKED__), keep in grid, skip clearing
  //   __CRACKED__ in a full line → destroy it (cleared to null), color stored as '__CRACKED__' for caller
  rf.forEach(r=>{for(let c=0;c<COLS;c++){
    const v=grid[r][c];
    if(v==='__BLOCKED__'){grid[r][c]='__CRACKED__';}// first hit: crack
    else{const k=r*100+c;cl2.add(k);clC[k]=v;} // __CRACKED__ or normal: clear
  }});
  cf.forEach(c=>{for(let r=0;r<ROWS;r++){
    const v=grid[r][c];
    if(v==='__BLOCKED__'){grid[r][c]='__CRACKED__';}
    else{const k=r*100+c;if(!cl2.has(k)){cl2.add(k);clC[k]=v;}}
  }});
  cl2.forEach(k=>{grid[Math.floor(k/100)][k%100]=null;});
  return{n:rf.length+cf.length,cells:[...cl2].map(k=>({r:Math.floor(k/100),c:k%100})),colors:clC};
}
